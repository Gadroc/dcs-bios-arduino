...
uint8_t DcsBiosRs485Device::pollingResponseBuffer[DCSBIOS_MAX_PACKET_DATA_SIZE];
...
void DcsBiosRs485Device::process() {
    ...
    do {
        ...
        if (_bus.getPacketState() == PACKET_COMPLETE 
            && _bus.getPacketType() == DCSBIOS_PACKETYPE_POLLING_REQUEST 
...
void DcsBiosRs485Device::sendPollResponse() {
    ...
    _busTxPin.set();
    _bus.sendPacket(_busStream, DCSBIOS_PACKETYPE_POLLING_RESPONSE, _address, pollingResponseBuffer, pollingResponseBufferSize);
    _busStream->flush();
    _busTxPin.clear();
...
void DcsBiosRs485Device::sendDcsBiosMessage(const char* message, const char* arg) {
    ...
    if (pollingResponseBufferSize + totalSize < DCSBIOS_MAX_PACKET_DATA_SIZE) {
